  ...
  useEffect(() => {
    const fetchSession = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to load solver session");
        }
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="space-y-4">
              {/* Settings Info */}
              {(session.kb_name || session.token_stats) && (
                <div className="flex flex-wrap gap-2 mb-4 pb-4 border-b border-slate-100 dark:border-slate-700">
                  ...
                  {session.token_stats?.cost !== undefined && session.token_stats.cost > 0 && (
                    <span className="px-2.5 py-1 text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-full">
                      Cost: ${session.token_stats.cost.toFixed(4)}
                    </span>
                  )}
                </div>
              )}
