...
const ALL_NAV_ITEMS: Record<string, { icon: LucideIcon; nameKey: string }> = {
  ...
  "/codeeditor": { icon: Code2, nameKey: "Code Editor" },
};
...
export default function Sidebar() {
  ...
  return (
    <div
      className="relative flex-shrink-0 bg-white/80 dark:bg-[#0f1729]/80 h-full border-r border-slate-200/70 dark:border-white/[0.06] flex flex-col transition-all duration-300 ease-in-out overflow-hidden backdrop-blur-sm"
      style={{ width: currentWidth }}
    >
      {/* Header */}
      <div
        className={`border-b border-slate-100/80 dark:border-white/[0.05] transition-all duration-300 ${sidebarCollapsed ? "px-2 py-3" : "px-4 py-3"
          }`}
      >
        <div className="flex flex-col gap-2">
          ...
          >
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0 bg-gradient-to-br from-indigo-500 to-violet-600 shadow-md shadow-indigo-500/20 relative">
                <Infinity className="w-6 h-6 text-white absolute bottom-[2px] opacity-90" strokeWidth={2.5} />
                <GraduationCap className="w-[18px] h-[18px] text-white absolute top-[2px] drop-shadow-md" strokeWidth={2.5} />
              </div>
              <h1
                className={`font-bold text-slate-800 dark:text-white tracking-tight text-base whitespace-nowrap transition-all duration-300 ${sidebarCollapsed
            ...
            >
              ...
              <button
                ...
                className="text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 p-1.5 hover:bg-slate-100 dark:hover:bg-white/[0.06] rounded-md transition-colors"
              ...
              <a
                ...
                className="text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 p-1.5 hover:bg-slate-100 dark:hover:bg-white/[0.06] rounded-md transition-colors"
                title="Visit ScholarLoop AI Homepage"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 p-1.5 hover:bg-slate-100 dark:hover:bg-white/[0.06] rounded-md transition-colors"
          ...
          >
            {isEditingDescription ? (
              ...
              <div
                ...
                className="text-[10px] font-medium text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-white/[0.04] px-2 py-1.5 rounded-md border border-slate-100 dark:border-white/[0.06] truncate cursor-pointer hover:bg-slate-100 dark:hover:bg-white/[0.06] hover:border-slate-200 dark:hover:border-white/[0.1] transition-colors group"
                title="Click to edit"
              >
      ...
      >
        {navGroups.map((group, idx) => (
          <div key={group.id}>
            ...
            <div
              className={`text-[10px] font-semibold text-slate-400/80 dark:text-slate-500 uppercase tracking-[0.08em] mb-2 truncate transition-all duration-300 ${sidebarCollapsed
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <Link
                      href={item.href}
                      className={`flex items-center rounded-lg transition-all duration-200 ${sidebarCollapsed
                          ? "justify-center p-2"
                          : "gap-2.5 pl-2.5 pr-1.5 py-2"
                        } ${isActive
                          ? "bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-l-[3px] border-l-indigo-500 dark:border-l-indigo-400 border border-indigo-100/50 dark:border-indigo-500/10"
                          : "text-slate-600 dark:text-slate-400 hover:bg-slate-100/80 dark:hover:bg-white/[0.04] hover:text-slate-900 dark:hover:text-slate-200 border border-transparent"
                        }`}
                      onMouseEnter={() =>
                    ...
                    >
                      <item.icon
                        className={`w-[18px] h-[18px] flex-shrink-0 transition-colors ${isActive
      ...
      {/* Footer */}
      <div
        className={`border-t border-slate-100/80 dark:border-white/[0.05] bg-white/40 dark:bg-white/[0.01] transition-all duration-300 ${sidebarCollapsed ? "px-2 py-2" : "px-2 py-2"
          }`}
      >
        <div className="relative">
          <Link
            href="/settings"
            className={`flex items-center rounded-lg text-sm transition-all duration-200 ${sidebarCollapsed
                ? "justify-center p-2"
                : "gap-2.5 pl-2.5 pr-1.5 py-2"
              } ${pathname === "/settings"
                ? "bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-l-[3px] border-l-indigo-500 dark:border-l-indigo-400"
          ...
          >
            <Settings
              className={`w-[18px] h-[18px] flex-shrink-0 transition-colors ${pathname === "/settings"
                  ? "text-indigo-500 dark:text-indigo-400"
        ...
        <button
          onClick={toggleSidebar}
          className={`w-full mt-2 flex items-center rounded-lg text-slate-400 dark:text-slate-500 hover:bg-slate-100/80 dark:hover:bg-white/[0.04] hover:text-slate-600 dark:hover:text-slate-300 transition-all duration-200 ${sidebarCollapsed ? "justify-center p-2" : "gap-2.5 pl-2.5 pr-1.5 py-2"
            }`}
