...
interface GlobalContextType {
  ...
  // UI Settings
  uiSettings: { theme: "light" | "dark"; language: "en" | "hi" };
  refreshSettings: () => Promise<void>;
  updateTheme: (theme: "light" | "dark") => Promise<void>;
  updateLanguage: (language: "en" | "hi") => Promise<void>;
...
export function GlobalProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateLanguage = async (newLanguage: "en" | "hi") => {
    // Update UI immediately
    ...
    const loadSidebarSettings = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.nav_order) {
            // Auto-merge: add any new items from DEFAULT_NAV_ORDER
            // that are missing from the saved order
            const merged = { ...data.nav_order };
            for (const groupKey of Object.keys(DEFAULT_NAV_ORDER) as Array<keyof SidebarNavOrder>) {
              if (!merged[groupKey]) {
                merged[groupKey] = DEFAULT_NAV_ORDER[groupKey];
              } else {
                const savedItems = new Set(merged[groupKey]);
                for (const item of DEFAULT_NAV_ORDER[groupKey]) {
                  if (!savedItems.has(item)) {
                    merged[groupKey] = [...merged[groupKey], item];
                  }
                }
              }
            }
            setSidebarNavOrderState(merged);
          }
        }
      } catch (e) {
